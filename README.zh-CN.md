...
- 所有开发任务必须从 `Leader` 开始。
- Leader 内置完整的 VS Code、执行、编辑、搜索、浏览器和已配置扩展工具；工具可用性不等于操作授权。
- Leader 默认不读取代码、不搜索、不执行命令，也不直接修改代码，只通过子代理获得结构化结果。
- 只有结果冲突或必须直接验证时，Leader 才能执行只读检查和验证命令。
- 只有用户在当前任务中明确要求 Leader 亲自操作且计划已批准精确范围时，Leader 才可例外写入。
- 批准前，Leader 只能调用只读 Analyzer 收集事实。
- Leader 输出计划后，必须等待用户准确回复：`批准执行`。
- 用户批准整体计划后，Leader 可自行组织并行任务、测试、审查和返工。
- 默认只有 Implementer 修改代码；Implementer 可在批准范围内执行格式化、构建、测试和静态检查等自验证。
- 最新批准计划逐项列出精确文件路径后，Implementer 可删除这些文件；单文件删除仍由 Hook 要求逐次确认，目录和递归删除继续阻断。
...
- 未在批准计划中逐项列出的文件删除、安装依赖、数据库迁移、配置变更和外部发布默认禁止；必要时由 Hook 要求显式确认或直接阻断。
