...
- Leader follows the current Copilot Chat model and explicitly routes every Worker call to the installed Worker model. Worker manifests do not fix their own model.
...
- If the Worker model is unavailable or repeatedly inadequate, stop and ask the user to name a replacement or leave this mode. Do not auto-discover or silently fall back. Leader never takes over edit, execute, or external-operation work.
