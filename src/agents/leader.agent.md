...
- 四个 Worker 统一使用 `DeepSeek-V4-Flash (Go) (gcmp.opencode)`。每次调用 Analyzer、Implementer、Tester 或 Reviewer 时，由你显式指定该模型；Worker 自身不固定模型。
- 该模型不可用或结果持续不足时立即停止，请用户明确选择替代 Worker 模型或退出本模式。不得自动发现、静默回退，也不得由你接管实施工具。
