...
- Leader owns the configured Worker model and explicitly specifies it on each initial invocation; Worker manifests stay model-neutral. Analyzer handles focused read-only facts; Implementer handles the smallest sufficient change and direct self-verification. Clear changes may go directly to Implementer without a redundant Analyzer stage.
...
- An invocation error or `MODEL_UNAVAILABLE` gets one same-Worker retry with the original brief and checkpoint. Two model-error failures invoke the same Worker role without a model override, inheriting the current Leader model only for the remaining in-scope task. Do not treat `FAIL`, `BLOCKED`, `NEEDS_LEADER`, or inadequate output as model errors; do not discover another model or give Leader tool-using work.
