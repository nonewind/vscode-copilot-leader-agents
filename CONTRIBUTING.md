...
- Leader owns the configured Worker model and explicitly specifies it on every invocation; Worker manifests stay model-neutral. Analyzer handles focused read-only facts; Implementer handles the smallest sufficient change and direct self-verification. Clear changes may go directly to Implementer without a redundant Analyzer stage.
...
- When the Worker model is unavailable or inadequate, Leader stops for explicit user model selection or asks the user to leave this mode; it never auto-discovers, silently falls back, or takes over tool-using work.
